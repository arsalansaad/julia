...
StepRange{T1,T2}(r::Range) where {T1,T2} =
...
(::Type{StepRange{T1,T2} where T1})(r::Range) where {T2} = StepRange{eltype(r),T2}(r)
...
StepRangeLen{T,R,S}(r::Range) where {T,R,S} =
    StepRangeLen{T,R,S}(R(first(r)), S(step(r)), length(r))
StepRangeLen{T}(r::Range) where {T} =
    StepRangeLen(T(first(r)), T(step(r)), length(r))
StepRangeLen(r::Range) = StepRangeLen{eltype(r)}(r)

promote_rule(a::Type{LinSpace{T1}}, b::Type{LinSpace{T2}}) where {T1,T2} =
    el_same(promote_type(T1,T2), a, b)
LinSpace{T}(r::LinSpace{T}) where {T} = r
LinSpace{T}(r::Range) where {T} = LinSpace{T}(first(r), last(r), length(r))
LinSpace(r::Range{T}) where {T} = LinSpace{T}(r)
...
Array{T,1}(r::Range{T}) where {T} = vcat(r)
